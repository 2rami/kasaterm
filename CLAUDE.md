...
카사텀은 터미널에서 여러 AI와 함께 일하는 앱이다.
보고와 핸드오프는 ‘끝난 것 / 남은 것 / 사용자가 할 것’만 짧게 쓴다. 막힌 일과 확인 못 한 것은 남은 것에 적는다.
현재 작업 상태는 [메모리 안내](.memory/MEMORY.md), 협업 방법은 [하네스 지침](app/kasaterm/collab-hooks/collab-protocol.md)에서 확인한다.
아래는 에이전트가 실행 전에 읽을 운영 상세다. 작업에 해당하는 안전규칙을 반드시 지킨다.
...
[!] 작업 시작 전 반드시 [.memory/MEMORY.md](.memory/MEMORY.md)와 관련 폴더 인덱스를 읽고, 거기서 연결하는 핸드오프와 토픽으로 현재 상태를 파악한다. 렌더러·색·아키텍처 배경, 렌더버그 카탈로그, 코드 수정 주의점은 관련 토픽에 있다.
