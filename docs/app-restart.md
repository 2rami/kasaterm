...
4. 대상 앱은 요청이 뭐라 하든 **나쵸에서 읽은 승인**(승인됨·조종 기기가 소비함·만료 전·자기 machine_id 와 지금 해시가
   scope 에 있음)과 **지금 자기 사실**(거부 사유 없음)로만 판정한다. 나쵸 키가 없는 기기는 승인을 쥔 기기(명부)의 카사텀이
   읽기만 중계한다(`GET /app/restart/approvals/{id}`) — 키는 그 기기 밖으로 안 나간다.
...
- 믿는다: 나쵸가 적은 승인 상태(서버 원자적 1회 소비), 대상 앱이 지금 잰 자기 사실.
