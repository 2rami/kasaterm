//! Listener thread + per-connection reader. Owns the socket file, the
//! accept loop, and the connect → newline-delimited JSON → dispatch
//! pipeline.
//!
//! Cross-platform note: this lives on `std::os::unix::net::UnixListener`
//! today. Windows ConPTY support arrives with Phase C — at that point a
//! `cfg(target_family = "windows")` branch will swap in a named-pipe
//! transport. The dispatch / framing layers above don't care which
//! socket type sits underneath.
...
impl Server {
    /// Bind to `socket_path`, deleting any stale socket file at that
    /// location first. Returns a server ready to be `spawn`-ed onto a
    /// background thread.
    pub fn bind(socket_path: impl Into<PathBuf>) -> Result<Self> {
        let socket_path = socket_path.into();
        // Stale socket from a previous crash — UnixListener::bind would
        // ENOADDRINUSE on top of an existing file, so clear it. The
        // failure modes we care about (perms, no parent dir) are caught
        // by the bind() call itself.
...
impl Drop for Server {
    fn drop(&mut self) {
        ...
        // behind. If the file is gone (someone else cleaned up first,
        // or the disk filled), ignore.
        let _ = std::fs::remove_file(&self.socket_path);
    }
}

fn handle_client(stream: UnixStream, backend: Arc<dyn Backend>) {
...
mod tests {
    ...
    fn ping_round_trip_over_real_socket() {
        // Bind a real Unix socket, send a single ping request frame,
