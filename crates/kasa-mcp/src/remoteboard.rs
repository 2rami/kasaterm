...
pub fn remotes() -> Vec<Remote> {
    ...
    // 한 base 가 /board 를 서빙하므로, board 가 보여주는
    // 기계와 이사가 아는 기계가 갈리지 않게 한 소스로 묶는다.
...
fn tag(mut row: Value, label: &str) -> Value {
    ...
    // 원격 세션에 닿는 길은 `tell` 하나다(보드 주소 + 그 기계의 `/collab/tell`, 2026-09-16
    // 유령 명부 걷음). 세션명이 있는 행만 `"tell"` — 셸 pane(peer_name 없음)은 받을
    // 하네스가 없어 `"remote"` 로 남겨 「보이되 안 닿음」을 정직하게 표식한다.
    match obj.get("peer_name").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
        Some(_) => {
            obj.insert("reach".into(), Value::String("tell".into()));
        }
        None => {
...
mod tests {
    ...
    fn 세션명_있는_원격행은_tell로_닿게_표식된다() {
        ...
        // 세션명은 그대로 싣고 reach 만 tell 로 —
        // board 를 읽는 오케스트레이터가 그 이름으로 SendMessage 하면 닿는다.
        assert_eq!(t["peer_name"], "hina-p1-abc");
        assert_eq!(t["reach"], "tell");
    ...
    fn 세션명_없는_원격행은_여전히_안_닿음으로_남는다() {
        // 셸 pane 은 받을 하네스가 없다 — reach="tell" 로 올리면 「보이는데 안 닿는」 stale.
