...
pub struct Turn {
    pub role: String,
    pub text: String,
    /// 이 턴에 첨부된 이미지 파일 경로(사용자가 붙여넣은 image block → 디코드 저장). 대화창 인라인.
...
/// 경로를 반환(사용자: 이미지도 대화창에). FNV 해시로 같은 이미지는 1회만 저장(dedup).
...
/// 추정 없이 질문/선택지를 API 그대로 얻는다(사용자: peek 추정 금지). input 은 도구의 원본
...
pub struct PaneConv {
    ...
    /// 디코드하면 잘린 청크를 통째 버려 한글 응답 streaming 이 0 이었다(사용자 실측).
    sse_bytes: Vec<u8>,
...
/// skills·환경 프리앰블이 통째로 붙어 채팅에 노이즈로 떴다(사용자). regex 없이 수동 스캔.
fn strip_meta(text: &str) -> String {
    ...
    for (open, close) in [
        ...
        // 사람이 친 게 아님(사용자: task-notification 이 선생님 말풍선으로 샜다).
        ...
        // commands…" 가 노란 선생님 말풍선에 raw 로 샜다(사용자 실측, 자주 발생).
    ...
    // image 블록(extract_images)으로 따로 렌더되므로 텍스트엔 노이즈(사용자: 말풍선에 경로 떴음).
...
/// 판별. 안 그러면 "프롬프트 제안" 호출의 SUGGESTION MODE 지시문이 채팅에 섞였다(사용자).
...
/// 덮어썼다(사용자) — `tools[]` 유무라는 구조적 시그널로 한 번 더 거른다. 둘 다 통과해야
...
/// 커스텀 등 명백한 서브는 잡아 대화창 혼입을 막는다(사용자). system 없으면 메인으로 본다.
    ...
    while let Some(nl) = conv.sse_buf.find('\n') {
        ...
        match v.get("type").and_then(|t| t.as_str()) {
            Some("content_block_start") => {
                ...
                // 를 모을 준비(content_block_stop 에서 완성). peek 화면 추정 대체(사용자).
...
pub async fn proxy_handler(
    ...
    for (k, val) in headers.iter() {
        ...
        // 찾아 streaming 이 0 이었다(사용자 실측: 답변이 다음 턴까지 안 뜸). 평문으로 받아야
...
mod tests {
    ...
    fn captures_only_main_conversation() {
        ...
        // 통째 교체되는 turns 를 "quota" 한 단어로 덮어쓴 실제 버그(사용자) → 스킵.
    ...
    fn strips_meta_preamble() {
        ...
        // 이미지 첨부 시 하네스가 박는 좌표 매핑 안내 줄 제거(사용자: 말풍선에 샜음)
        assert_eq!(
            strip_meta(
    ...
    fn sse_feed_handles_utf8_split_midchar() {
        // 한글 응답이 청크 경계에서 멀티바이트 중간으로 잘리는 실측 케이스(사용자) —
