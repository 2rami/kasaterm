...
/// 그 채움 위에 **두를 테두리 색** — 카드·메뉴·배지의 가장자리.
///
/// `border()` 를 그냥 쓰면 안 되는 건 그 토큰이 한 자리에 고정된 색이라서다.
/// 어두운 판 위에서는 채움과 같은 대역으로 내려앉아 그대로 사라지고, 밝은 판
/// 위에서는 되레 튄다 — 층을 선언하라고 두른 선이 정작 층이 필요한 자리에서만
/// 침묵했다(2026-08-27 지적 「테두리나 모양도 신경써줘」). 채움에서 글자색 쪽으로
/// 조금 끌어오면 밝은 테마에서는 어두운 선이, 어두운 테마에서는 밝은 선이 되어
/// 여덟 테마 어디서든 한 줄로 남는다.
pub fn edge_on(fill: [u8; 4]) -> [u8; 4] {
    lerp(fill, text(), 0.22)
}
