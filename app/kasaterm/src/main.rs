...
/// 방 카드 머리 아래 배치도(미니맵)의 높이. 목록은 "누가 있나"만 말하고 **어느 칸이
/// 화면 어디인지**는 못 말한다 — pane 이 예닐곱이면 제목만 보고는 못 고른다.
///
/// 46px 은 두 단 분할까지 칸이 알아볼 만하게 남는 최소치다. 더 키우면 정작 목록이
/// 밀려 카드가 화면을 넘고(넘친 방은 아예 안 그려진다), 더 줄이면 3분할부터 칸이
/// 선처럼 뭉갠다.
const SIDEBAR_MINI_H: f32 = 46.0;
