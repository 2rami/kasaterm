    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // 남았다 — 거노: "누를때마다 바뀐다는 표시가 없고".
        {
            ...
            std::thread::spawn(move || {
                ...
                // 비활성은 정해진 간격에 한 번이다.
                //
                // 그 간격이 5분에서 **1분**으로 내려왔다(2026-08-27 지시). 5분은 이
                // 값이 드롭다운을 열었을 때만 보이던 시절의 간격이다 — 여는 순간
                // 최신이면 됐다. 이제 하단바가 나머지 계정을 **상시** 세우므로 5분
                // 묵은 숫자는 「실시간으로 계속 보인다」와 어긋난다. 활성 계정과 같은
                // 박자로 맞춘다: 슬롯 넷이면 upstream 호출이 분당 4회고, 레이트리밋에
                // 닿는 양은 아니다.
                let mut others_at: Option<std::time::Instant> = None;
                const OTHERS_EVERY: std::time::Duration = std::time::Duration::from_secs(60);
                loop {
