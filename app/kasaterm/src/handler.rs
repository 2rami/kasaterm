    ...
    fn user_event(&mut self, event_loop: &ActiveEventLoop, event: UserEvent) {
        ...
        match &event {
            ...
            UserEvent::SocketMigrateBack(pane, cwd, force, reply) => {
                #[cfg(unix)]
                let outcome = self
                    .migrate_pane_back(pane, cwd.as_deref(), *force)
                    .map_err(|e| format!("{e:#}"));
                #[cfg(not(unix))]
                let outcome: std::result::Result<String, String> =
                    Err("이사는 unix 전용이다".to_string());
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        // 남았다 — 거노: "누를때마다 바뀐다는 표시가 없고".
        {
            ...
            std::thread::spawn(move || {
                ...
                // 때마다 깨우면 사이클이 훨씬 자주 도는데, 카운터로 세면 그만큼 간격이
                // 짧아져 비활성 슬롯 조회가 덩달아 잦아진다 — 정작 upstream 부담을
                // 아끼려고 둔 간격이 조용히 무너진다. 시각으로 재면 몇 번을 깨우든
                // 비활성은 정해진 간격에 한 번이다.
                //
                // 간격은 5분이다. 2026-08-27 에 1분으로 내렸는데, 그건 하단바가
                // 나머지 계정을 **상시** 세우던 시절의 근거였다 — 이제 그 줄은
                // 활성 하나만 세우고 나머지는 눌러야 보이므로(2026-09-07) 상시
                // 신선할 이유가 없다.
                //
                // 되돌리는 진짜 이유는 429 다. 슬롯 넷을 1분마다 치면 upstream 이
                // 레이트리밋으로 막고, 그러면 **활성 계정의 숫자까지** 함께 못 읽어
                // 하단바가 통째로 빈칸이 된다(2026-09-07 실측: 앱 로그가 429 로
                // 덮여 있었다). 나머지 슬롯을 아끼는 것이 활성 슬롯을 지키는 길이다.
                // 드롭다운을 열면 그 자리에서 한 바퀴 돌므로(`usage_poke`) 볼 때는
                // 최신이다.
                ...
                loop {
                    ...
                    //
                    // 비활성 슬롯은 `OTHERS_EVERY`(1분)마다 친다. 매 사이클 치면
                    // 만료된 슬롯에 헛 curl 을 계속 띄우고, 살아 있는 슬롯은 upstream
                    // 레이트리밋을 활성 계정과 나눠 쓰게 된다 — 그 사이는 6시간짜리
                    // 디스크 스냅샷이 메운다.
