...
impl App {
    ...
    fn route_account_action_to_home(&mut self, action: &SettingsAction) -> bool {
        ...
        // 사라진다 — 2026-09-05 에 실제로 그렇게 만들어 사용자가 「설정창이랑 하단이랑
        // 왜 다르냐」고 물었다.
    ...
    pub(crate) fn settings_apply(&mut self, action: SettingsAction) {
        ...
        match action {
            ...
            SettingsAction::FileOpenMode(m) => {
                ...
                // "OS 연결 프로그램" 인데, 이 기기의 기본은 사용자가 목록에서 일부러
            ...
            SettingsAction::RemoveClaudeAccount(id) => {
                ...
                // 보인다(사용자 2026-08-13: "슬롯정리가 안 되던데" — 실제로 지운
    ...
    pub(crate) fn settings_web_action(
        ...
        match action {
            ...
            // 창은 키가 WKWebView 로 가 그 경로에 닿지 않는다(사용자 2026-08-15
...
/// (사용자 2026-09-05 「설정창에 붙여넣는거 없이도 가능하게 해봐」).
///
...
///
/// 격리만 있던 동안 불편이 실재했다(사용자 2026-08-15 「계정 로그인이 쓰던 브라우저로
...
///
/// **다시 로그인의 기본은 `Default`(쓰던 브라우저)다**(사용자 2026-08-15 「orca 처럼
...
/// (사용자 2026-08-15 「다시 로그인 말고 추가여도 그냥 쓰던 브라우저여도 되고」).
///
/// 여기는 원래 격리 고정이었다. 이유가 있었다 — 쓰던 브라우저를 열면 그 창의
/// claude.ai 세션이 그대로 승인돼 슬롯 전부가 같은 계정이 됐다(사용자: "계정추가하면
...
/// 것을 Orca 방식으로 바꾼 것이다(사용자 2026-08-13 「로그인방식도 ㄱ」).
///
...
mod login_url_tests {
    ...
    /// 빈 크롬 창을 다시 보게 된다(사용자 2026-08-15 「쓰던 브라우저로 안 열림」).
    #[test]
...
/// 이메일이 방금 로그인한 계정으로** 바뀌었다(사용자: "계정추가하면 1도 그거로 바뀌어").
...
pub(crate) struct AuthProbe {
    ...
    /// (사용자: "팀플랜인지 구분하게 돼?"). 한도가 따로 도는 별개 계정이라 이걸
...
/// 안 그린다. TTL 을 두는 이유: 사용자가 pane 에서 `/login` 을 마치면 클릭 없이
...
/// 계정이 주기적으로 풀리는 것처럼 깜빡였다(사용자 2026-08-03). git 폴러가 일시적
...
/// 계정 슬롯을 부를 이름. 사용자가 별명을 직접 붙였으면 그것, 아니면 그 슬롯의
/// **진짜 이메일**.
///
/// "기본"·"계정 2" 같은 자동 이름은 슬롯을 하나도 구별해 주지 않는다 — 계정
/// 행에도 드롭다운에도 전환 토스트에도 그 이름이 떠서, 정작 어느 계정으로
/// 갈아탔는지는 아무 데도 안 적혀 있었다(사용자 요청 2026-08-06). 이메일은
...
/// 한도를 안 쓰고 있었다(사용자 2026-08-07: "3개가 모두 같을리가없는데").
///
...
/// 예전 버전이 추가할 때 파일에 박아 둔 것이라 사용자가 친 별명이 아니다.
