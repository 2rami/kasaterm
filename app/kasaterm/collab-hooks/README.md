...
사용자가 직접 오케스트레이터 — claude 끼리 자율 협업/통솔 안 함. board 는 호출 시점 pull(다른 pane 활동 보기), 충돌은 `conflict-guard` 가 transcript 직접 비교로 자동 차단, 메시지는 SendMessage(claude pane)·`kasaterm-cli tell`(그 외). 가시성은 GUI(SCHALE OS) 관찰 레이어가 담당.
