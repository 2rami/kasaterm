...
board 전체 + inbox 를 밀어 넣어 워커 컨텍스트를 부풀렸고 그래서 걷어냈다(사용자 06-14).
...
는 목록을 본 뒤 표시 이름을 설명한다. 목록 자체는 바꾸지 않으며, 전송 주소는
별도로 board --all에서 확인하도록 안내한다. 표시 이름을 전송 주소로 쓰지 않는다.
...
def on_list_agents(d):
    ...
    emit(
        ...
        "이 기계에서 캐릭터가 잡히는 것들:\n"
        + pairs
        + "\n부를 때는 캐릭터 이름을 쓴다. 이 표는 표시용이며 전송 주소가 아니다. "
        "`kasaterm-cli board --all`에서 기기·방·신원을 확인한 최신 address 전체로 "
        "`kasaterm-cli tell --address '<주소 JSON>' --stdin`을 사용한다. "
        "사용자가 정한 범위만 맡기고, 위험·취향·범위 질문은 사용자에게 직접 한다." + warn,
    )
