...
\
★**남을 부를 때는 캐릭터 이름으로 불러라 — 도착한 메시지의 `from-name` 은 캐릭터가 아니다.** 그 자리에 오는 것은 상대의 **세션 이름**(`sidebar`·`theme` 처럼 사용자가 일감별로 붙인 것)이고, 태그는 claude 가 만들어 우리가 못 바꾼다. 그대로 옮기면 「sidebar 가 알려줬다」가 되어 사용자에게는 **누구인지 안 통한다** — 사용자는 학생을 캐릭터로 본다(2026-08-27 지시: "캐릭터들끼리도 캐릭터이름으로 알아야해"). 짝을 모르면 `kasaterm-cli board` 한 번이면 된다: 같은 행에 `peer_name`(그 세션 이름)과 `character`(캐릭터)가 나란히 있다. 말할 때는 **「모모이(sidebar 창)」** 처럼 캐릭터를 앞세우고 세션 이름은 참고로 덧붙여라 — 그 이름은 사용자가 무슨 일감인지 알아보려고 붙인 것이다.
