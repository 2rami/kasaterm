...
카사텀 자체의 PC·모바일 화면을 설계·시안·구현할 때 따른다.

- HTML 대체 화면과 Claude 아티팩트를 만들지 않는다. 시안도 같다.
- PC는 Rust+wgpu/winit 네이티브, 모바일은 Flutter 실제 위젯이다. 시안은 격리 실행한 실제 화면의 스크린샷으로 보인다.
- 이미 있는 별도 웹 제품은 이 규칙을 이유로 지우지 않는다.
