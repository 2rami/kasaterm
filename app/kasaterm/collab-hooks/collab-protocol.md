...
`activity` 는 한 pane 을 지목해 보므로 안 자른다 — 사람이 시킨 것 · 그 학생이 말한 것 · 부른 도구와 **인자 원문** · 돌아온 결과와 **오류 여부**가 시간순으로 온다. 동료가 막힌 것 같은데 이유를 모를 때, 화면(`peek`)이나 대화(`transcript`)보다 이쪽이 빠르다 — 그 둘은 무엇을 쳤는지를 안 준다.
⚠️ **평소에 activity 를 돌리지 마라.** 한 사람 것이 board 전체보다 크다. 이상해 보이는 **한 명**을 지목할 때만 — 전원에게 돌리면 네 창이 먼저 마른다.
⚠️ board 의 도구 라벨은 `설명 — 명령` 꼴이다(2026-08-30). 앞의 설명은 그 학생이 붙인 한 줄이라 같은 명령을 반복해도 서로 갈린다.
...
\
★**남을 부를 때는 캐릭터 이름으로 불러라 — 도착한 메시지의 `from-name` 은 캐릭터가 아니다.** 그 자리에 오는 것은 상대의 **세션 이름**(`sidebar`·`theme` 처럼 사용자가 일감별로 붙인 것)이고, 태그는 claude 가 만들어 우리가 못 바꾼다. 그대로 옮기면 「sidebar 가 알려줬다」가 되어 사용자에게는 **누구인지 안 통한다** — 사용자는 학생을 캐릭터로 본다(2026-08-27 지시: "캐릭터들끼리도 캐릭터이름으로 알아야해"). 짝은 **대개 저절로 알려준다** — 메시지가 도착한 턴과 `ListAgents` 를 본 직후에 훅(`kasaterm-peer-name.py`)이 「이 이름은 이 캐릭터다」를 붙여 준다. 그게 안 왔으면(다른 기계의 세션이거나 board 가 늦은 것이다) `kasaterm-cli board` 한 번이면 된다: 같은 행에 `peer_name`(그 세션 이름)과 `character`(캐릭터)가 나란히 있다. ⚠️**주소는 여전히 세션 이름이다** — `to:` 에 캐릭터 이름을 넣으면 닿지 않는다. 그리고 같은 캐릭터가 두 창에 앉아 있을 수 있으니, 그때는 세션 이름을 함께 말해라. 말할 때는 **「모모이(sidebar 창)」** 처럼 캐릭터를 앞세우고 세션 이름은 참고로 덧붙여라 — 그 이름은 사용자가 무슨 일감인지 알아보려고 붙인 것이다.
...
나쵸네코는 사내 슬랙에 사는 봇이고, 학생과 서로 지시를 주고받는다(2026-08-27 확립).
- **나쵸→학생**: 나쵸가 kasaterm send 로 네 pane 에 직접 브리프를 넣는다 — 그냥 받으면 된다.
- **학생→나쵸**: 나쵸 DM(`D0BSEDCGU66`)에 storm `slack_send` 로 **거노 계정을 빌려** 남긴다. 머리에 「(○○가 남김 — 거노 계정 빌림)」을 붙이고, **매번 거노 문안 승인을 받고 보내라** — 거노 이름으로 사내에 나가는 글이다.
...
[협업 — 학생 스폰]
★**넘길지는 「손이 많으냐」가 아니라 「내 창을 태우느냐」로 정한다.** 스크린샷·실사이트 확인, 파일 수십 개 훑기, 긴 HTML·문서 옮기기처럼 **읽어 들이는 양이 결과보다 큰 일**은 네가 한가해도 넘겨라 — 같은 방의 노는 학생이나 값싼 창(glm·kimi 탭)에게. 2026-09-03 지적: 방2 학생이 화면 개편을 스크린샷 확인까지 혼자 하다 창이 압축됐고, 그 방엔 노는 학생이 둘 있었다 — 08-24 의 「idle상태면 굳이 학생소환안하고 본인이 하게해줘」가 「웬만하면 띄우지 마라」로 읽혀 창을 태우는 일까지 직접 하게 된 것이다. 그 지시의 뜻은 **한가하다고 남을 띄우지는 마라**이지, 넘길 일을 끌어안으라는 것이 아니다. 순서는 이렇다:
1. **창을 태우는 일이면 넘긴다.** 같은 방에 노는(idle) 학생이 있으면 `kasaterm-cli board` 에서 찾아 SendMessage 로 브리프를 보낸다 — 이미 뜬 학생은 맥락과 pane 이 공짜고 거노 화면도 안 는다(단 `detached` 는 제외 — 위 절). 없으면 아래처럼 값싼 창을 탭으로 띄운다. 한 학생에게 같은 영역 일을 연달아 주는 것이 소환 여럿보다 낫다. ⚠️**같은 방(팀) 학생만이다** — board 는 모든 방을 보여 주지만, 다른 방의 idle 은 그 방 프로젝트의 대기 인력이지 공용 풀이 아니다. 방은 거노가 프로젝트별로 가른 화면이고 태스크 목록·맥락도 방 단위라, 다른 방 학생을 끌어오면 그 방의 대기 맥락이 깨지고 거노는 브랜딩 창에서 엉뚱한 일이 도는 것을 보게 된다(2026-08-24 지적 「세나는 브랜딩이고 다른방인데 왜 시켰어」 — 즉시 회수했다). 맞는 idle 이 같은 방에 없으면 2·3번으로 가라.
...
- **이름은 학생이 직접 붙인다** — 띄운 직후 네가 `rename %N` 을 걸면 표시가 안 남아 한 턴 뒤 되돌아간다. 브리프만 보내면 학생이 자기 절(위)대로 스스로 붙인다.
...
[협업 — 내 pane 에 이름 붙이기]
**브리프를 받으면 첫 일로 이름을 붙여라** — claude 의 `/rename <무슨 일인지 4~10자>`. 사용자는 pane 여럿을 한 화면으로 본다. **학생 이름만 떠 있으면 어느 창이 무슨 일인지 알 길이 없다.** 「로고 시안 재작업」·「모바일 레이아웃 수정」처럼 **무엇을 하는지**로 짧게 쓰고, 주제가 크게 바뀌면 다시 붙여라. 이 이름은 사이드바 줄과 **Alt(옵션)를 눌렀을 때 pane 번호 아래**에도 뜬다 — 안 붙이면 그 자리엔 학생 이름만 남고, 창이 여럿일 때 그것만으론 어느 게 무슨 일인지 못 가른다.
...
★**첫 명령은 `nlm --ai` 다.** `nlm --help` 는 명령 이름만 나열하고 쓰는 법을 안 준다 — 그것만 보면 「못 쓰겠다」 싶어 익숙한 길로 새게 된다. 2026-08-26 실측: 거노가 「nlm cli ㄱㄱ」라고 콕 집어 지시했는데도 학생이 `--help` 만 찍어 보고 `curl` 로 제목을, `yt-dlp --list-subs` 로 자막을 뽑으며 네 턴을 샜다. `--ai` 를 읽은 그 자리에서 바로 제대로 갔다.
