...
[협업 — 학생 스폰]
★**스폰은 마지막 수단이다**(거노 지시 2026-08-24 「idle상태면 굳이 학생소환안하고 본인이 하게해줘」). 일감이 생기면 이 순서다:
1. **`kasaterm-cli board` 에서 노는(idle) 학생부터 찾아 SendMessage 로 새 브리프를 보낸다** — 이미 뜬 학생은 맥락과 pane 이 공짜고 거노 화면도 안 는다(단 `detached` 는 제외 — 위 절). 한 학생에게 같은 영역 일을 연달아 주는 것이 소환 여럿보다 낫다. ⚠️**같은 방(팀) 학생만이다** — board 는 모든 방을 보여 주지만, 다른 방의 idle 은 그 방 프로젝트의 대기 인력이지 공용 풀이 아니다. 방은 거노가 프로젝트별로 가른 화면이고 태스크 목록·맥락도 방 단위라, 다른 방 학생을 끌어오면 그 방의 대기 맥락이 깨지고 거노는 브랜딩 창에서 엉뚱한 일이 도는 것을 보게 된다(2026-08-24 지적 「세나는 브랜딩이고 다른방인데 왜 시켰어」 — 즉시 회수했다). 맞는 idle 이 같은 방에 없으면 2·3번으로 가라.
2. **작거나 까다로운 일은 네가 직접 해라** — 파일 한둘 고치기, 갈림길 판단, 몇 분짜리 수정은 브리프 왕복이 일 자체보다 크다. 위임할 것은 손이 많은 일이지 어려운 일이 아니다.
3. **새로 띄우는 것은 맞는 idle 이 없을 때만** — 전원이 바쁘거나, 컨텍스트가 말라 가거나, 남은 창이 거노 답 대기뿐일 때.
