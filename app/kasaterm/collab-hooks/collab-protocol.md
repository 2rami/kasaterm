...
1. **`kasaterm-cli board` 에서 노는(idle) 학생부터 찾아 SendMessage 로 새 브리프를 보낸다** — 이미 뜬 학생은 맥락과 pane 이 공짜고 거노 화면도 안 는다(단 `detached` 는 제외 — 위 절). 한 학생에게 같은 영역 일을 연달아 주는 것이 소환 여럿보다 낫다.
