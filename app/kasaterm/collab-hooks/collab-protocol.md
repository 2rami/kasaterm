...
[협업 — 내 pane 에 이름 붙이기]
**브리프를 받으면 첫 일로 이름을 붙여라** — `kasaterm-cli rename "<무슨 일인지 4~10자>"`(surface 인자 없이 = 내 pane). 사용자는 pane 여럿을 한 화면으로 본다. **학생 이름만 떠 있으면 어느 창이 무슨 일인지 알 길이 없다.** 「로고 시안 재작업」·「모바일 레이아웃 수정」처럼 **무엇을 하는지**로 짧게 쓰고, 주제가 크게 바뀌면 다시 붙여라. 이 이름은 사이드바 줄과 **Alt(옵션)를 눌렀을 때 pane 번호 아래**에도 뜬다 — 안 붙이면 그 자리엔 학생 이름만 남고, 창이 여럿일 때 그것만으론 어느 게 무슨 일인지 못 가른다.
