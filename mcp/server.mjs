...
tool('browser_new_window', 'Open a SEPARATE Chrome window instead of adding a tab to the human\'s existing window. Prefer this over browser_new_tab when you will open several pages, or when the human asked you not to disturb their tabs — their tab bar keeps exactly the tabs and order it had. Pass width+height to check a layout at a specific size (more accurate than emulation, which resets on the next navigation). Pass tabId to tear an existing tab out into its own window. Opens UNFOCUSED by default so it does not cover whatever app the human is using; pass focused:true only when the page must actually be visible (animation, media, rAF). Close it with browser_close_window — that closes every tab inside, so do not point it at the human\'s own window. When opened from a url the tab lands in your own tab group (returned as groupId); a tab torn out with tabId keeps whatever group it had, since it may be the human\'s. Calling this repeatedly does NOT pile up windows: if you already have one open it opens a tab there instead and answers with reused:true, resizing that window if you asked for a size. That is almost always what you want — a fresh window per check leaves the human with a screen full of windows and a tab group per window to clear. Pass reuse:false when you genuinely need a second window side by side. Windows you open are yours to clean up — close them as soon as you are done, and before you finish the task.', {
