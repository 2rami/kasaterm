...
#
# 그리고 **작품마다 정상 범위가 다르다.** 위 값은 블루아카 실측이고, 원작이 단색인
# 작품에는 그대로 못 쓴다 — 치이카와의 검은 별·검은 마녀·회색 갑옷은 그림이
# 완벽해도 색 5~15·런 10 으로 나와 전부 탈락했다(2026-08-24 실측). 같은 배치에서
# 진짜 뭉갠 둘은 런 84·155 로 자릿수가 달랐으니, 문턱만 작품에 맞추면 갈라진다.
MIN_COLORS = int(os.environ.get("THEME_MIN_COLORS", "16"))
MAX_RUN = float(os.environ.get("THEME_MAX_RUN", "4"))
ATTEMPTS = 3
...
def gen_profile(slug, force=False):
    ...
    if not os.path.exists(ref):
        return slug, "no-ref", "theme-wiki.py 로 포트레이트를 먼저 받아라"
    # 프사엔 무기를 넣는다(2026-08-20 사용자 지시) — 기존 12명 프사가 전부 공식
