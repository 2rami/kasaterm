...
기계를 명부에 적어 두면 **원격 탭**에서 방별로 학생을 보고, 학생 하나를 「거울」로 옆에 앉히거나 기계 하나의 학생 전부를 방 단위로 거울로 펼치고, 「화면 보기」로 그 기계의 화면공유를 연다. Info 탭 「다른 기계」 줄이 그 기계의 학생 수·기다림·거울 수를 요약한다. 원격 pane은 몸통에 색 리본이 붙어 헤더를 안 봐도 갈린다.
